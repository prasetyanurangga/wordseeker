...
class Puzzle extends Equatable {
  ...
  List getNumberOfCorrectTilesPerRow() {
    final whitespaceTile = getWhitespaceTile();
    var numberOfCorrectTiles = 0;
    var numberOfCorrectTilesPerRow = [];
    for( var i = 0 ; i < getDimension(); i++ ) {
      numberOfCorrectTilesPerRow.add(0);
    }
    for (final tile in tiles) {
      if (tile != whitespaceTile) {
        ...
        if ((tile.correctPosition.y == tile.currentPosition.y && tile.correctPosition.x == tile.currentPosition.x) || bunchTilesOther.length > 0) {
          numberOfCorrectTilesPerRow[tile.currentPosition.y-1]++;
        }
      }
    }

    print(numberOfCorrectTilesPerRow);
    return numberOfCorrectTilesPerRow;
  }
