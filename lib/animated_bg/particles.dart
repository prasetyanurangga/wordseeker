...
abstract class ParticleBehaviour extends Behaviour {
  ...
  void paint(PaintingContext context, Offset offset) {
    ...
    for (Particle particle in particles!) {
      ...
      if (_particleImage != null) {
        ...
        textPainter.layout(
          minWidth: 0,
          maxWidth: particle.radius,
        );
        canvas.save();
        final pivot = textPainter.size.center(Offset(particle.cx, particle.cy)); 
        canvas.translate(pivot.dx, pivot.dy); 
        canvas.rotate(particle.angle);
        canvas.translate(-pivot.dx, -pivot.dy);
        textPainter.paint(canvas, Offset(particle.cx, particle.cy));
        canvas.restore();
      }
    }
  }
  ...
  List<Particle> generateParticles(int numParticles) {
    return List.generate(numParticles, (i) => i).map((i) {
      ...
      var randomStr = getRandomString(1);
      Particle p = Particle();
      p.text = randomStr;
      initParticle(p);
      return p;
